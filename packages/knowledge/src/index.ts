...
export const HINTS: Hint[] = [
  ...
  },
  {
    id: 'art-of-keeping',
    kind: 'kokoro',
    source: 'pattern-15 生かす技術としてのアート思考',
    text: 'この家に残っているもののなかで、捨てるつもりだったけれど、じつは面白いものはありませんか。古い建具、味噌樽、家紋の入った瓦、使わなくなった冷蔵庫。設計から考えるのではなく、そこに「あるもの」から考えてみると、買っては手に入らないものが場の顔になります。100年もったのは、100年大切にした人がいたからです。',
    when: (s) => s.hasModel && !s.hasPlans,
  },
  {
    id: 'bring-what-you-have',
    kind: 'kokoro',
    source: 'pattern-17 なんでも持ち寄ってもらう',
    text: 'いま足りないのは、お金でしょうか、技術でしょうか、それとも人でしょうか。足りないものを一人で埋めようとする前に、周りに「これ余っていない？」と聞いてみてください。道具も、資材も、腕も、たいてい誰かの家で眠っています。借りたぶんだけ、その人はこの場を自分の場所だと思うようになります。',
    when: (s) => s.hasPlans && s.todoDone < s.todoTotal,
  },
  {
    id: 'details',
    kind: 'kokoro',
    source: 'pattern-18 細部をつくり込む',
    text: '仲間と手を動かすと、どうしても仕上げの粗さがばらつきます。どこか一箇所だけ、無駄に見えるほど丁寧につくる場所を決めてみてください。手すりの touch、棚板の面取り、戸の引き手。人はそこを触って、この場の性格を受け取ります。',
    when: (s) => s.hasPlans && s.pinCount >= 0 && s.todoDone > 0,
  },
  {
    id: 'this-town-is-this-town',
    kind: 'kokoro',
    source: 'pattern-19 偶然を呼び込む',
    text: 'この街は、あなたがいてもいなくても、この街であり続けます。理想の街を探し続けるより、いまここにある制約を正しく引き受けたほうが、はやく始められます。そう思えると、失敗しても街は壊れないので、思い切って動けます。',
    when: (s) => !s.hasModel && !s.hasDiagnosis,
  },
  {
    id: 'toward-the-light',
    kind: 'kokoro',
    source: 'pattern-23 ひかりの方へ',
    text: '地域のためを思うほど、頼まれごとが増えて、何でも屋になっていきます。これまでやってきたことを踏み台にしたとき、いちばん遠くまで行けるテーマはどれでしょうか。課題を潰すのではなく、そこから新しい価値が生まれる方向を選んでください。',
    when: (s) => s.hasPlans && s.todoDone >= 2,
  },
  {
    id: 'shared-goal',
    kind: 'kokoro',
    source: 'pattern-26 増課題まちづくり',
    text: '日々の連絡は取れていても、一緒に街を面白くしている感じまでは、まだ遠いかもしれません。この家で解けそうな地域の困りごとをひとつ選んで、みんなの共通の的として言葉にしてみてください。的があると、外の人も入ってこられます。',
    when: (s) => s.hasPlans && s.todoDone >= 3,
  },
  {
    id: 'beyond-the-town',
    kind: 'kokoro',
    source: 'pattern-27 場の力で巻き込み続ける',
    text: '地域の中だけで回そうとすると、お金も担い手もいずれ細ります。この家から始まって街全体が手を取り合っている、と外から見て分かる場になっているでしょうか。そう見えたとき、街の外から来た人が、次の担い手になります。',
    when: (s) => s.hasPlans && s.todoDone >= 4,
  },
  {
    id: 'learn-from-the-ground',
    kind: 'kokoro',
    source: 'pattern-21 地に学ぶ',
    text: 'この家の思い出を、近所の人に聞きましたか。誰が住んでいて、何をしていた家か。同じ話をきっかけに、この土地の建材や、この辺りの大工の手を知ることができます。調べものだけでは出てこない話が、隣の家の縁側にあります。',
    when: (s) => s.hasModel && s.pinCount === 0,
  },
];
